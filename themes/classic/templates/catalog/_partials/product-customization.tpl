...
<section id="showshow-woop" class="product-customization" style="">
  {if !$configuration.is_catalog}
    <div id="custompdt" class="off">
      ...
      <h3 class="h4 card-title customtitle">{l s='Product customization' d='Shop.Theme.Catalog'}</h3>
      {block name='product_customization_form'}
        <form method="post" action="{$product.url}" enctype="multipart/form-data">
          <ul class="clearfix">
            {foreach from=$customizations.fields item="field"}
              <li class="product-customization-item">
                ...
                {if $field.type == 'text'}
                  <textarea placeholder="{l s='Your message here' d='Shop.Forms.Help'}" class="product-message" maxlength="24" {if $field.required} required {/if} name="{$field.input_name}"></textarea>
                  <small class="float-xs-right">{l s='24 char. max' d='Shop.Forms.Help'}</small>
                  {if $field.text !== ''}
          ...
          <div class="clearfix">
            <button class="btn btn-primary float-xs" type="submit" name="submitCustomizedData">{l s='Save Customization' d='Shop.Theme.Actions'}</button>
          </div>
        </form>
      {/block}
