...
class statssales extends ModuleGraph
{
    ...
    protected function getData($layers)
    {
        ...
        $this->query = '
			SELECT o.`date_add`, o.`total_paid_real` / o.conversion_rate as total_paid_real, SUM(od.product_quantity) as product_quantity
			FROM `'._DB_PREFIX_.'orders` o
			LEFT JOIN `'._DB_PREFIX_.'order_detail` od ON od.`id_order` = o.`id_order`
    ...
    protected function setAllTimeValues($layers)
    {
        ...
        foreach ($result as $row) {
            if ($this->option == 1) {
                $this->_values[0][(int)substr($row['date_add'], 0, 4)] += 1;
                $this->_values[1][(int)substr($row['date_add'], 0, 4)] += $row['product_quantity'];
            } else {
                $this->_values[(int)substr($row['date_add'], 0, 4)] += $row['total_paid_real'];
            }
        }
    }
    ...
    protected function setMonthValues($layers)
    {
        ...
        foreach ($result as $row) {
            if ($this->option == 1) {
                $this->_values[0][(int)substr($row['date_add'], 8, 2)] += 1;
                $this->_values[1][(int)substr($row['date_add'], 8, 2)] += $row['product_quantity'];
            } else {
                $this->_values[(int)substr($row['date_add'], 8, 2)] += $row['total_paid_real'];
            }
        }
    }
    ...
    protected function setDayValues($layers)
    {
        ...
        foreach ($result as $row) {
            if ($this->option == 1) {
                $this->_values[0][(int)substr($row['date_add'], 11, 2)] += 1;
                $this->_values[1][(int)substr($row['date_add'], 11, 2)] += $row['product_quantity'];
            } else {
                $this->_values[(int)substr($row['date_add'], 11, 2)] += $row['total_paid_real'];
            }
        }
    }
