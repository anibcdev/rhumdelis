...
class AdminGformrequestexportController extends ModuleAdminController
{
    public function __construct()
	{
		...
		$this->display = 'edit';
        parent::__construct();
		$this->meta_title = $this->module->l('Advanced Form Builder');
	}
    public function initToolBarTitle()
    {
        $this->toolbar_title[] = $this->module->l('Advanced Form Builder');
        $this->toolbar_title[] = $this->module->l('Request Export');
    }
    ...
    public function renderForm(){
       ...
       if($forms)
        ...
        $allStatus = array(
            array('value' => 0,'name' => $this->module->l('Submitted')),
            array('value' => 1,'name' => $this->module->l('Pending')),
            array('value' => 2,'name' => $this->module->l('Closed')),
        );
        $this->fields_form = array(
			'legend' => array(
				'title' => $this->module->l('Export Csv'),
				'icon' => 'icon-export'
			),
			'input' => array(
				array(
                ...
                array(
					'type' => 'checkbox',
					'label' => $this->module->l('Request Status'),
					'name' => 'status',
					'values' => array(
						'query' => $allStatus,
						'id' => 'value',
		...
		$this->toolbar_title = $this->module->l('Export');
    ...
    public function postProcess()
	{
        if (Tools::isSubmit('submitExport'))
        {
            ...
            if(count($allStatus_submit) < 1){
                ...
                $titles['gdate_add'] = $this->module->l('Date Add');
                fputcsv($fp, $titles);
                if($results){
