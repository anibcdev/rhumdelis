...
<div class="payment_module systempay {$systempay_tag|escape:'html':'UTF-8'}">
  <a href="javascript: $('#systempay_paypal').submit();" title="{l s='Click here to pay with Paypal' mod='systempay'}">
    <img class="logo" src="{$systempay_logo|escape:'html':'UTF-8'}" alt="Systempay" />{$systempay_title|escape:'html':'UTF-8'}

    <form action="{$link->getModuleLink('systempay', 'redirect', array(), true)|escape:'html':'UTF-8'}" method="post" id="systempay_paypal">
      <input type="hidden" name="systempay_payment_type" value="paypal" />
    </form>
  </a>
</div>
