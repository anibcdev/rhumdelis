...
if (! class_exists('SystempayRequest', false)) {
    ...
    class SystempayRequest
    {
        ...
        {
            // initialize encoding
            $this->encoding = in_array(Tools::strtoupper($encoding), SystempayApi::$SUPPORTED_ENCODINGS) ?
                Tools::strtoupper($encoding) : 'UTF-8';
            ...
            $timestamp = time();
            $this->set('vads_trans_id', SystempayApi::generateTransId($timestamp));
        ...
        {
            $this->requestParameters[$name] = new SystempayField($name, $label, $regex, $required, $length);
        ...
        {
            $this->redirectEnabled = ($enabled && $enabled != '0' && Tools::strtolower($enabled) != 'false');
            return true;
        }
        ...
        /**
         * Generate signature from a list of SystempayField.
         *
         * @param array[string][SystempayField] $fields already filtered fields list
         * @param bool $hashed
         * @return string
         */
        private function generateSignature($fields, $hashed = true)
        {
            ...
            return SystempayApi::sign($params, $this->getCertificate(), $this->algo, $hashed);
        }
         ...
         *
         * @return array[string][SystempayField] a list of SystempayField
         */
        ...
        public function getRequestUrl()
        {
            ...
            $url = Tools::substr($url, 0, - 1); // remove last &
            return $url;
        }
