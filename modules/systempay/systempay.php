...
require_once _PS_MODULE_DIR_.'systempay/classes/SystempayFileLogger.php';
require_once _PS_MODULE_DIR_.'systempay/classes/SystempayTools.php';

require_once _PS_MODULE_DIR_.'systempay/classes/payment/AbstractSystempayPayment.php';
require_once _PS_MODULE_DIR_.'systempay/classes/payment/SystempayAncvPayment.php';
require_once _PS_MODULE_DIR_.'systempay/classes/payment/SystempayChoozeoPayment.php';
require_once _PS_MODULE_DIR_.'systempay/classes/payment/SystempayFullcbPayment.php';
require_once _PS_MODULE_DIR_.'systempay/classes/payment/SystempayMultiPayment.php';
require_once _PS_MODULE_DIR_.'systempay/classes/payment/SystempayOneyPayment.php';
require_once _PS_MODULE_DIR_.'systempay/classes/payment/SystempayPaypalPayment.php';
require_once _PS_MODULE_DIR_.'systempay/classes/payment/SystempaySepaPayment.php';
require_once _PS_MODULE_DIR_.'systempay/classes/payment/SystempaySofortPayment.php';
require_once _PS_MODULE_DIR_.'systempay/classes/payment/SystempayStandardPayment.php';
...
class Systempay extends PaymentModule
{
    ...
    public function install()
    {
        ...
        // set default values
        foreach (SystempayTools::getAdminParameters() as $param) {
            if (in_array($param['key'], SystempayTools::$multi_lang_fields)) {
                $default = SystempayTools::convertIsoArrayToIdArray($param['default']);
        ...
        if (SystempayTools::$plugin_features['oney'] && !Configuration::get('SYSTEMPAY_OS_ONEY_PENDING')) {
            ...
            $oney_state->name = SystempayTools::convertIsoArrayToIdArray($name);
            ...
            @copy(
                _PS_MODULE_DIR_.'systempay/views/img/os_oney.gif',
                _PS_IMG_DIR_.'os/'.Configuration::get('SYSTEMPAY_OS_ONEY_PENDING').'.gif'
            );
        }

        if (!Configuration::get('SYSTEMPAY_OS_TO_VALIDATE')) {
            ...
            $tvp_state->name = SystempayTools::convertIsoArrayToIdArray($name);
            ...
            if (!$tvp_state->save() || !Configuration::updateValue('SYSTEMPAY_OS_TO_VALIDATE', $tvp_state->id)) {
                return false;
            }

            // add small icon to state
            @copy(
                _PS_MODULE_DIR_.'systempay/views/img/os_tvp.gif',
                _PS_IMG_DIR_.'os/'.Configuration::get('SYSTEMPAY_OS_TO_VALIDATE').'.gif'
            );
        }

        if (!Configuration::get('PS_OS_OUTOFSTOCK_PAID') && !Configuration::get('SYSTEMPAY_OS_PAYMENT_OUTOFSTOCK')) {
            ...
            $oos_state->name = SystempayTools::convertIsoArrayToIdArray($name);
            ...
            // add small icon to state
            @copy(
                _PS_MODULE_DIR_.'systempay/views/img/os_oos.gif',
                _PS_IMG_DIR_.'os/'.Configuration::get('SYSTEMPAY_OS_PAYMENT_OUTOFSTOCK').'.gif'
            );
        }

        if (!Configuration::get('SYSTEMPAY_OS_AUTH_PENDING')) {
            ...
            $auth_state->name = SystempayTools::convertIsoArrayToIdArray($name);
            ...
            if (!$auth_state->save() || !Configuration::updateValue('SYSTEMPAY_OS_AUTH_PENDING', $auth_state->id)) {
                return false;
            }

            // add small icon to state
            @copy(
                _PS_MODULE_DIR_.'systempay/views/img/os_auth.gif',
                _PS_IMG_DIR_.'os/'.Configuration::get('SYSTEMPAY_OS_AUTH_PENDING').'.gif'
            );
        }

        if ((SystempayTools::$plugin_features['sofort'] || SystempayTools::$plugin_features['sepa'])
            && !Configuration::get('SYSTEMPAY_OS_TRANS_PENDING')) {
            ...
            $sofort_state->name = SystempayTools::convertIsoArrayToIdArray($name);
            ...
            if (!$sofort_state->save() || !Configuration::updateValue('SYSTEMPAY_OS_TRANS_PENDING', $sofort_state->id)) {
                return false;
            }

            // add small icon to state
            @copy(
                _PS_MODULE_DIR_.'systempay/views/img/os_trans.gif',
                _PS_IMG_DIR_.'os/'.Configuration::get('SYSTEMPAY_OS_TRANS_PENDING').'.gif'
            );
        }
    ...
    public function uninstall()
    {
        ...
        // delete all obsolete Systempay params but not custom order states
    ...
    {
        require_once _PS_MODULE_DIR_.'systempay/classes/SystempayRequest.php';
        ...
        foreach (SystempayTools::getAdminParameters() as $param) {
            ...
            $name = isset($param['name']) ? $param['name'] : null; // Systempay API parameter name
            ...
            if (in_array($key, SystempayTools::$multi_lang_fields)) {
                if (!is_array($value) || empty($value)) {
                    $value = array();
                }
            } elseif (in_array($key, SystempayTools::$group_amount_fields)) {
                if (!is_array($value) || empty($value)) {
                    $value = array();
                } else {
                    $error = false;
            ...
            } elseif ($key === 'SYSTEMPAY_STD_PAYMENT_CARDS' || $key === 'SYSTEMPAY_MULTI_PAYMENT_CARDS') {
            ...
            } elseif (in_array($key, SystempayTools::$amount_fields)) {
                if (!empty($value) && (!is_numeric($value) || $value < 0)) {
                    $this->_errors[] = sprintf($this->l('Invalid value «%1$s» for field «%2$s».'), $value, $label);
                    continue;
                }
            } elseif ($key === 'SYSTEMPAY_STD_CARD_DATA_MODE' && $value == '3' && !Configuration::get('PS_SSL_ENABLED')) {
                ...
                continue;
            } elseif (($key === 'SYSTEMPAY_STD_PROPOSE_ONEY') && ($value == 'True')) {
                $oney_enabled = Tools::getValue('SYSTEMPAY_ONEY_ENABLED', 'False') == 'True' ? true : false;
            ...
            } elseif (($key === 'SYSTEMPAY_ONEY_OPTIONS') && (Tools::getValue('SYSTEMPAY_ONEY_ENABLE_OPTIONS', 'False') == 'True')) {
                if (!is_array($value) || empty($value)) {
                    $value = array();
                } else {
                    $error = false;
                ...
                $value = serialize($value);
            } elseif (($key === 'SYSTEMPAY_FULLCB_OPTIONS') && (Tools::getValue('SYSTEMPAY_FULLCB_ENABLE_OPTS', 'False') == 'True')) {
                if (!is_array($value) || empty($value)) {
                    $value = array();
                } else {
                    $error = false;
    ...
    private function validateOney($inside = false)
    {
        ...
        if (!$inside) {
            $group_amounts = Tools::getValue('SYSTEMPAY_ONEY_AMOUNTS');
    ...
    private function renderForm()
    {
        ...
        require_once _PS_MODULE_DIR_.'systempay/classes/admin/SystempayHelperForm.php';

        $this->context->smarty->assign(SystempayHelperForm::getAdminFormContext());
        $form = $this->context->smarty->fetch(_PS_MODULE_DIR_.'systempay/views/templates/admin/back_office.tpl');

        $prefered_post_vars = 0;
        $prefered_post_vars += substr_count($form, 'name="SYSTEMPAY_');
    ...
    public function hookHeader($params)
    {
        ...
        if ($controller instanceof OrderController || $controller instanceof OrderOpcController) {
            if (isset($this->context->cookie->systempayPayErrors)) {
                // process errors from other pages
                $controller->errors = array_merge(
                    $controller->errors,
                    explode("\n", $this->context->cookie->systempayPayErrors)
                );
                unset($this->context->cookie->systempayPayErrors);
    ...
    public function hookPayment($params)
    {
        ...
        $tag = version_compare(_PS_VERSION_, '1.6', '<') ? 'systempay15' : 'systempay16';
        $this->context->smarty->assign('systempay_tag', $tag);
        ...
        $choozeo = new SystempayChoozeoPayment();
        ...
        $sofort = new SystempaySofortPayment();
    ...
    public function hookPaymentOptions($params)
    {
        ...
        // version tag for specific styles
        $this->context->smarty->assign('systempay_tag', 'systempay17');

        /**
         * AbstractSystempayPayment::getPaymentOption() returns a payment option of type
        ...
        if ($standard->isAvailable($cart)) {
            ...
            if ($standard->hasForm()) {
                $this->context->smarty->assign($standard->getTplVars($cart));
                $form = $this->fetch('module:systempay/views/templates/hook/'.$standard->getTplName());

                if ($standard->getEntryMode() == '4') {
                    // iframe mode
                    $option->setAdditionalInformation($form);
                    $option->setForm('<form id="systempay_standard" onsubmit="javascript: return false;"></form>');
                } else {
                    $option->setForm($form);
                }
            }
        ...
        if ($multi->isAvailable($cart)) {
            ...
            if ($multi->hasForm()) {
                $this->context->smarty->assign($multi->getTplVars($cart));
                $form = $this->fetch('module:systempay/views/templates/hook/'.$multi->getTplName());
                $option->setForm($form);
            }
        ...
        if ($choozeo->isAvailable($cart)) {
            ...
            if ($choozeo->hasForm()) {
                $this->context->smarty->assign($choozeo->getTplVars($cart));
                $form = $this->fetch('module:systempay/views/templates/hook/'.$choozeo->getTplName());
                $option->setForm($form);
            }
        ...
        if ($oney->isAvailable($cart)) {
            ...
            if ($oney->hasForm()) {
                $this->context->smarty->assign($oney->getTplVars($cart));
                $form = $this->fetch('module:systempay/views/templates/hook/'.$oney->getTplName());
                $option->setForm($form);
            }
        ...
        if ($fullcb->isAvailable($cart)) {
            ...
            if ($fullcb->hasForm()) {
                $this->context->smarty->assign($fullcb->getTplVars($cart));
                $form = $this->fetch('module:systempay/views/templates/hook/'.$fullcb->getTplName());
                $option->setForm($form);
            }
    ...
    {
        if (isset($this->context->cookie->systempayPartialRefundWarn)) {
            $this->context->controller->warnings[] = $this->context->cookie->systempayPartialRefundWarn;
            unset($this->context->cookie->systempayPartialRefundWarn);
        }
    }
    ...
    public function hookActionOrderSlipAdd($params)
    {
        if (Tools::isSubmit('partialRefund') && ($params['order']->module == $this->name)) {
            ...
            $this->context->cookie->systempayPartialRefundWarn = $msg;
        }
    }
     ...
     * @param Cart $cart
     * @param int $state
     * @param SystempayResponse $response
     * @return Order
     */
    ...
    public function savePayment($order, $response)
    {
        ...
        if ($response->get('card_brand') == 'MULTI') {
            ...
            $multi_options = SystempayMultiPayment::getAvailableOptions();
    ...
    private function addOrderPayment($order, $invoice, $trans_id, $amount, $timestamp, $data)
    {
        ...
        // update transaction info if payment is modified in Systempay Back Office
    ...
    public static function nextOrderState($response, $total_refund = false, $outofstock = false)
    {
        if ($response->isAcceptedPayment()) {
            ...
            switch (true) {
                ...
                default:
                    ...
                    if (($response->get('operation_type') === 'CREDIT') && $total_refund) {
                        ...
                        $new_state = 'SYSTEMPAY_OS_TRANS_PENDING';
                    } else {
            ...
            if ($outofstock) {
                if ($valid) {
                    $new_state = Configuration::get('PS_OS_OUTOFSTOCK_PAID') ? 'PS_OS_OUTOFSTOCK_PAID' : 'SYSTEMPAY_OS_PAYMENT_OUTOFSTOCK';
                } else {
    ...
    public static function isSuccessState($order)
    {
        ...
        $s_states = array(
            ...
            'SYSTEMPAY_OS_TO_VALIDATE',
            'SYSTEMPAY_OS_ONEY_PENDING',
            'SYSTEMPAY_OS_AUTH_PENDING'
        );
    ...
    public static function isOutOfStock($order)
    {
        ...
        $oos_states = array(
            ...
            'PS_OS_OUTOFSTOCK', // considered as pending by Systempay module for PrestaShop < 1.6.1
