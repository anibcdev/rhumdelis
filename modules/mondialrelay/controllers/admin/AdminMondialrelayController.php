...
class AdminMondialRelayController extends ModuleAdminController
{
    ...
    private function displayOrdersTable()
    {
        ...
        $MRCreateTicket = new MRCreateTickets(
            array(
                'orderIdList' => null,
                'totalOrder'  => null,
                'weightList'  => null
...
//        echo $this->mondialrelay->fetchTemplate('/views/templates/admin/', 'settings');
    ...
    public function display()
    {
        ...
        if (MondialRelay::isAccountSet() && (int)$this->mondialrelay->account_shop['MR_ORDER_STATE']) {
            $this->context->smarty->assign(array('isAccountSet' => true));
            $this->displayOrdersTable();
            $this->displayhistoriqueForm();
        } else {
            $this->context->smarty->assign(array('isAccountSet' => false));
        }
        $path = 'views/templates/admin/';
        $name = 'admin_mondial_relay';
        $this->template = '../../../../modules/mondialrelay/'.$path.$name.'.tpl';
        parent::display();
    }
    ...
    {
        MondialRelay::debug('install tab called');
        $tab             = new Tab();
        $tab->active     = 1;
        $tab->name       = array();
        ...
        foreach (Language::getLanguages(true) as $lang) {
            $tab->name[$lang['id_lang']] = 'Mondial Relay';
        }
