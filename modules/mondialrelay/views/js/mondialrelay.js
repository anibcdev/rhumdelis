...
$(document).ready(function() {
    ...
    if ($('body').hasClass('adminmondialrelay')) {
        $('#content').removeClass('bootstrap');
        $('#content').addClass('nobootstrap');
    }
    $(document).on('click', '.trMRSelected *', function(event) {
...
// List the marker liable to the relay pint
        ...
        $.ajax(
            {
                ...
                data    : {
                    'detailedExpeditionList': detailedExpeditionList,
                    'method': 'MRGetTickets',
                    'mrtoken': mrtoken
                },
                dataType: 'json',
            ...
            if ((++i >= $('.PS_MRSuccessList').length) && json && json.success) {
                for (id_order in json.success)
                    if (json.success[id_order] && json.success[id_order].expeditionNumber) {
                        ...
                        detailedExpeditionList.push({
                            'id_order'        : id_order,
                            'expeditionNumber': json.success[id_order].expeditionNumber
                        });
    ...
    function checkToDisplayRelayList() {
        if (typeof PS_MRData != 'undefined') {
            ...
            $('input[name^=delivery_option]').each(function (i, e) {
                ...
                var parent_element = $(e).parents('.delivery-option').find('label[for^=delivery_option] > div');
