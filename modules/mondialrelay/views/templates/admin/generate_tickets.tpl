...
<fieldset>
	...
	<form method="post" action="{$smarty.server.REQUEST_URI|escape:'htmlall':'UTF-8'}" class="MR_form_admintab">
		...
		{foreach from=$MR_orders key=case_num item=order}
			<tr id="PS_MRLineOrderInformation-{$order.id_order|intval}">
				...
				<td>
					<input type="text" name="weight_{$order.id_order|intval}" id="weight_{$order.id_order|intval}" size="7" value="{$order.weight|escape:'htmlall':'UTF-8'}" />
				</td>
				<td>
				...
				<td class="center">
					<a href="index.php?tab=AdminOrders&id_order={$order.id_order|intval}&vieworder&token={$MR_token_admin_orders|escape:'htmlall':'UTF-8'}">
						<img border="0" title="{l s='View' mod='mondialrelay'}" alt="{l s='View' mod='mondialrelay'}" src="{$new_base_dir|escape:'htmlall':'UTF-8'}views/img/details.gif"/>
					</a>
				</td>
			</tr>
	...
	{if !$MR_orders|count}
		...
		<div class="submit_button">
			...
			<div class="PS_MRLoader" id="PS_MRSubmitGenerateLoader"><img src="{$new_base_dir|escape:'htmlall':'UTF-8'}views/img/getTickets.gif"</div>
		</div>
	{/if}
	</form>
</fieldset>
