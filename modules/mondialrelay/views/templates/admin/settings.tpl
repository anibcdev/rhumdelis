...
<div>
    <form action="{$smarty.server.REQUEST_URI|escape:'htmlall':'UTF-8'}" method="post" class="form">
        <fieldset>
            <legend><img src="../modules/mondialrelay/views/img/logo.gif"/>{l s='Admin Tab Settings' mod='mondialrelay'}
            </legend>
                ...
                <select id="id_order_state" name="id_order_state" style="width:250px">
                    {foreach from=$MR_orders_states_list key=num_state item=order_state}
                        ...
                        <option value="{$order_state.id_order_state|intval}"
                                style="background-color:{$order_state.color|escape:'htmlall':'UTF-8'};" {$selected_option|escape:'htmlall':'UTF-8'}>{$order_state.name|escape:'htmlall':'UTF-8'}</option>
                    {/foreach}
                </select>
                <p>
